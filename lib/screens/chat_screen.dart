...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    ...
    // ⭐⭐ 로컬 알림 서비스에도 채팅방 진입 알림 (2회당 1회 카운터 초기화)
  ...
  void _startSecuretDirectly() async {
    ...
    try {
      ...
      final otherUserQrUrl = userDoc.data()?['qrUrl'] as String?;
      final otherUserNickname = userDoc.data()?['nickname'] as String? ?? '알 수 없음';
      
      // QR URL 검증
      if (otherUserQrUrl == null || otherUserQrUrl.isEmpty) {
        if (!mounted) return;
        _showSnackBar(
          '${otherUserNickname}님이 아직 Securet을 등록하지 않았습니다',
          isError: true,
        );
        return;
      }
      ...
      if (!otherUserQrUrl.startsWith('http://') && !otherUserQrUrl.startsWith('https://')) {
        if (!mounted) return;
        _showSnackBar('잘못된 Securet URL 형식입니다', isError: true);
        return;
      }
  ...
  Widget _buildImageMessage(String imageUrl, bool isMe) {
    // 🐱 스티커 판별 (Firebase Storage의 stickers 폴더 또는 외부 스티커 URL)
    final bool isSticker = imageUrl.contains('/stickers/') || 
